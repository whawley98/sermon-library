...
export default function StatsPage({ pastor }) {
  ...
  if (!stats) {
    return (
      <div className="stats-page-wrap">
        <div className="stats-empty"><p>No statistics available yet.</p></div>
      </div>
    );
  }
              ...
              {topKw.slice(0, 20).map(({ word, count }) => (
                <div key={word} className="bar-row">
                  ...
                  <div className="bar-track">
                    <div className="bar-fill" style={{ width: `${(count / maxKw) * 100}%` }} />
                  </div>
            ...
            <div className="bar-chart">
              {topBooks.slice(0, 20).map(({ book, count }) => (
                <div key={book} className="bar-row">
                  ...
                  <div className="bar-track">
                    <div className="bar-fill bar-fill-rust" style={{ width: `${(count / maxBook) * 100}%` }} />
                  </div>
                  <span className="bar-count">{count}</span>
                </div>
              ))}
